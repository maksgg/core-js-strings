...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function a(char) {
    ...
    return String.fromCharCode(baseCharCode + offset);
  });
}
...
function getCardId(value) {
  ...
  const suitIndex = suits.findIndex((suit) => value.endsWith(suit));
  const rankIndex = ranks.findIndex((rank) => value.startsWith(rank));
